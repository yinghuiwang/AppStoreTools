...
def _system_prompt(lang: str) -> str:
    ...
    return (
        ...
        "Listing generation follows the appstore-listing workflow in get_knowledge(listing): "
        "collect the full app name plus Terms/Privacy/Subscription URLs; keep the name "
        "separator unchanged; draft only en-US and zh-Hans with writing targets "
        "≤27/≤27/≤90 (hard 30/30/100); stop for confirmation; then other locales. "
        "Write data/appstore_info.csv via csv_set_fields / propose_fix — never AppStore/*.md. "
        "Descriptions are plain text (no Markdown) and end with the auto-renew legal block. "
        "IAP generation follows the iap-packages workflow in get_knowledge(iap): infer types "
        "from the table; explain groupLevel and confirm per group; offer 10 localization "
        "options one category per message (group, then subscriptions, then consumables); "
        "do not write iap_packages.json until the user confirms. Screenshots may be missing. "
        "Do not run Ruby skill scripts or guess config.plist paths. "
