...
def _metadata_locale_presence(profile: str) -> tuple[list[str], bool]:
    """Return present locale codes. Never raises; failures degrade presence only."""
    if not profile:
        return [], False
    try:
        config = Config(app_name=profile)
        ...
        if not version:
            return [], False
        codes = [item["locale"] for item in _get_available_locales(api, app_id)]
        return codes, True
...
def metadata_locales(request: Request):
    """Return the static locale catalog without calling App Store Connect.

    Presence badges are filled by ``GET /metadata/locales/presence``.
    """
    lang = _lang(request)
    try:
    ...
    return {
        "locales": _absent_locales(catalog),
        "presenceAvailable": False,
    }


@router.get("/metadata/locales/presence")
def metadata_locales_presence(request: Request):
    """Return locale codes present on the current editable version.

    Sync ``def`` so ASC presence checks stay off the event loop.
    Guard/ASC failures degrade to 200 + presenceAvailable false (never 409).
    """
    codes, presence_available = _metadata_locale_presence(_cookie_profile(request))
    return {"codes": codes, "presenceAvailable": presence_available}
...
# IAP HTTP routes live in routes_iap.py (mounted at /api/iap).
# Re-export starters/handlers so existing tests and inspect() keep working.
from asc.web.routes_iap import (  # noqa: E402
    _scan_iap_review_screenshot_targets,
    _start_iap_compare_task,
    _start_iap_review_screenshots_task,
    _start_iap_task,
    iap_check,
    iap_compare,
    iap_review_screenshots_scan,
    iap_review_screenshots_upload,
    iap_run,
)
